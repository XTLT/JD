/**
 * jd_mall.js（重写版）—— 浏览商品保活 任务查询
 * functionId: apTaskList   appid: activities_platform   linkId: H1GZjgq
 */
const { callBff, loadCookie } = require('../jd_core.js');

async function main() {
  const cfg = loadCookie();
  console.log(`[mall] 账号 ${decodeURIComponent(cfg.pt_pin)}`);
  const r = await callBff('apTaskList', { linkId: 'H1GZjgq' }, { appid: 'activities_platform' });
